import { json, newSessionToken, readJson, sessionCookieName, setCookie, verifyPassword, } from '../_lib/auth.js';
import { kvEnvIssue, kvGet, kvSet } from '../_lib/store.js';
export default async function handler(req, res) {
    res.setHeader('Cache-Control', 'no-store');
    const issue = kvEnvIssue();
    if (issue)
        return json(res, 500, { ok: false, error: issue });
    if (req.method !== 'POST') {
    ...
    if (!username || !password)
        return json(res, 400, { ok: false, error: 'MISSING' });
